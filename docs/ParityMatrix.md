...
| OCR / screen snip | `ScreenSnipper.swift` | New Windows impl using Snipping Tool protocol callback, shared-storage token redemption, `file-access-token` parsing, callback-file storage fallback, and Windows OCR bridge | `Baseline done: WSA-RT-011; callback flow: WSA-RT-011B; diagnostics: WSA-RT-011C; file-access-token callback: WSA-RT-011D; callback storage fallback: WSA-RT-011E` |
