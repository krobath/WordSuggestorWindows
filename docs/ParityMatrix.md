...
| Toolbar text-to-speech action | `FloatingToolbarView.speakSelection()` + `SpeechHighlighter.swift` | Windows-native selected/staged text speech with external selection mirroring, source-aware Windows voice selection, diagnostics, hotkey path, and editor reading highlight | `Baseline done: WSA-RT-013; voice fallback done: WSA-RT-013A; external selection/highlight pass: WSA-RT-013B; OneCore catalog/runtime in progress: WSA-RT-013D; highlight visibility/restore done: WSA-RT-013E` |
