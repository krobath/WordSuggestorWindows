...
| Toolbar text-to-speech action | `FloatingToolbarView.speakSelection()` + `SpeechHighlighter.swift` | Windows-native selected/staged text speech with external selection mirroring, language-aware SAPI voice selection, and diagnostics | `Baseline done: WSA-RT-013; voice fallback done: WSA-RT-013A` |
...
| Full app text to speech feature | `NeuralTTSService.swift` / system TTS | New Windows impl through toolbar-level SAPI bridge with per-language voice override baseline | `Baseline done: WSA-RT-013; language voice selection: WSA-RT-013A` |
