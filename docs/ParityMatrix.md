...
| Toolbar text-to-speech action | `FloatingToolbarView.speakSelection()` + `SpeechHighlighter.swift` | Windows-native selected/staged text speech with external selection mirroring, language-aware SAPI voice selection, diagnostics, hotkey path, and editor reading highlight | `Baseline done: WSA-RT-013; voice fallback done: WSA-RT-013A; external selection/highlight pass: WSA-RT-013B` |
...
| Full app text to speech feature | `NeuralTTSService.swift` / system TTS | New Windows impl through toolbar-level SAPI bridge with per-language voice override baseline, external selection hotkey, and editor reading highlight | `Baseline done: WSA-RT-013; language voice selection: WSA-RT-013A; selection/highlight pass: WSA-RT-013B` |
